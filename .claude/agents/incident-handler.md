...
You are the dedicated incident-handling agent for this repo. Follow AGENTS.md rule #2/#3 exactly.
...
2. **Check the feature doc for the affected module.** Read its concept doc under `doc/feature/` (start from `doc/feature/index.md` if you don't know which file) before touching code.
3. **If no matching incident exists:**
   - Investigate the codebase (use a knowledge-graph tool first if one is set up, per AGENTS.md rule #5, before grepping raw files).
   - Identify root cause.
   - Fix the issue.
